...
class PoeCrafting(private val catalog: PoeCatalog, private val random: Random = Random.Default) {
    ...
    fun generate(baseId: String, itemLevel: Int, rarity: PoeRarity, stockReferences: List<ru.descend.domain.modifiers.ModifierRef>? = null): PoeItem {
        ...
        require((rarity == PoeRarity.UNIQUE) == catalog.unique(base)) { "Rarity must match the base's curated unique definition" }
        val item = PoeItem(baseId, itemLevel, rarity, (stockReferences ?: base.strings("implicits").map { ru.descend.domain.modifiers.ModifierRef(it, catalog.revision(it)) }).map {
        ...
        return when (rarity) {
            PoeRarity.UNIQUE -> item.copy(explicits = base.strings("unique_mods").map { id ->
                require(catalog.enabled(id)) { "Disabled unique modifier: $id" }; roll(id) })
    ...
    private fun fill(item: PoeItem, target: Int): PoeItem {
        var result = item
        repeat((target - item.explicits.size).coerceAtLeast(0)) {
            // Low level / compact pools can have fewer valid groups. Never fail a valid drop.
            if (eligible(result).isEmpty()) return result
            result = add(result)
        }
        return result
    }
    fun validate(item: PoeItem) {
        ...
        if (item.rarity == PoeRarity.UNIQUE) {
            require(catalog.unique(catalog.base(item.baseId))) { "Missing curated unique definition" }
            require(item.explicits.map { it.id } == catalog.base(item.baseId).strings("unique_mods")) { "Unique modifier set mismatch" }
            require(item.explicits.none { it.fractured }) { "Unique items cannot be fractured" }
        } else {
        require(!catalog.unique(catalog.base(item.baseId))) { "Unique base requires UNIQUE rarity" }
    ...
    fun apply(item: PoeItem, currency: PoeCurrency): PoeItem {
        ...
        require(item.rarity != PoeRarity.UNIQUE || currency in setOf(PoeCurrency.DIVINE, PoeCurrency.BLESSED)) {
            "Only Divine and Blessed Orbs can modify curated unique items"
        }
        ...
        val result = when (currency) {
            ...
            PoeCurrency.DIVINE -> {
                rarity(PoeRarity.MAGIC, PoeRarity.RARE, PoeRarity.UNIQUE)
