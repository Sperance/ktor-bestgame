...
import ru.descend.domain.modifiers.ModifierScope
import ru.descend.domain.modifiers.ModifierSource
import ru.descend.domain.modifiers.ModifierTag
...
class PoeCatalog(val bases: Map<String, JsonObject>, val mods: Map<String, JsonObject>,
    ...
    companion object {
        ...
        val tierMetadata: Map<String, JsonObject> by lazy {
            val stream = requireNotNull(PoeCatalog::class.java.getResourceAsStream("/poe/tiers.json.gz"))
            GZIPInputStream(stream).bufferedReader().use { Json.parseToJsonElement(it.readText()).jsonObject.mapValues { e -> e.value.jsonObject } }
        }
        ...
        fun definitionFromRaw(id: String, raw: JsonObject): ModifierDefinition {
            ...
            return ModifierDefinition(id = id, name = raw.string("text").ifBlank { raw.string("name").ifBlank { id } },
                ...
                tiers = listOf(ModifierTier(tierMetadata[id]?.int("tier", 1) ?: 1, raw.int("required_level", 1).coerceAtLeast(1), 100,
    ...
    init {
        ...
        bases.forEach { (id, base) ->
            require((base.strings("implicits") + base.strings("unique_mods")).all { it in mods }) { "Missing implicit on $id" }
        }
        mods.forEach { (id, mod) ->
            require(mod.objects("stats").all { it.int("min") <= it.int("max") }) { "Invalid stat range: $id" }
        }
    }
    fun base(id: String) = requireNotNull(bases[id]) { "Base $id is outside the active compact catalog; retain inventory and expand the reviewed catalog to craft it" }
    fun revision(id: String): Int = heads[id] ?: 1
    fun enabled(id: String) = id !in disabled
    fun unique(base: JsonObject) = base.strings("unique_mods").isNotEmpty()
    ...
    fun legacyModifier(roll: PoeRoll, implicit: Boolean = false): Modifier {
        ...
        return Modifier(roll.id, roll.values.map { ModifierValue(it.toDouble()) }, tierMetadata[roll.id]?.int("tier", 1) ?: 1,
            if (implicit) ModifierSource.BASE_ITEM else if (definition.string("generation_type") == "unique") ModifierSource.UNIQUE else if (definition.string("generation_type") == "prefix") ModifierSource.PREFIX else ModifierSource.SUFFIX,
