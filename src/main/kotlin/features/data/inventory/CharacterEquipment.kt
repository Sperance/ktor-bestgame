...
data class CharacterEquipment(
    ...
    /**
     * Копия, снятая Зеркалом Каландры. Как в POE, её тоже больше не изменить.
     *
     * Отдельное поле, а не порча: запрет один и тот же, но состояния разные -
     * порченый предмет прошёл через Ваал, неизменяемый был кем-то скопирован,
     * и на карточке это две разные строки с разными символами.
     */
